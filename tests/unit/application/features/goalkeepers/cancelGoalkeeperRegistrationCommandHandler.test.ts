...
function buildHandler() {
  ...
  const handler = new CancelGoalkeeperRegistrationCommandHandler(sender, repository);
  return { repository, sender, handler };
}

describe('CancelGoalkeeperRegistrationCommandHandler', () => {
  it('deletes both document photos and the registration itself', async () => {
    const { handler, repository, sender } = buildHandler();
    const registration = GoalkeeperRegistration.createEmpty('reg-1', 'user-1');
    ...
    const result = await handler.handle(new CancelGoalkeeperRegistrationCommand('user-1'));
  ...
  it('is a graceful no-op when nothing was ever saved', async () => {
    ...
    const result = await handler.handle(new CancelGoalkeeperRegistrationCommand('user-2'));
  ...
  it('does not attempt to delete photos that were never uploaded', async () => {
    const { handler, repository, sender } = buildHandler();
    const registration = GoalkeeperRegistration.createEmpty('reg-1', 'user-3');
    registration.savePhysicalData({ heightCm: 185, weightKg: 78 });
    repository.seed(registration);

    await handler.handle(new CancelGoalkeeperRegistrationCommand('user-3'));
  ...
  it('refuses to cancel an active profile', async () => {
    ...
    const active = GoalkeeperRegistration.createEmpty('reg-active', 'user-4');
    active.activate();
    repository.seed(active);

    const result = await handler.handle(new CancelGoalkeeperRegistrationCommand('user-4'));
