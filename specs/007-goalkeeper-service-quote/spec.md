...
- **Configuration missing at both levels**: If neither the city nor its country defines the booking window, the minimum notice or the surcharge tiers, or the country has no currency, the quote is refused with a reason saying the service is not configured for that area; nothing is assumed and no price is returned.
...
- **Country without a currency**: If the country has no currency defined, the quote is refused as not configured, naming the currency as the missing setting; a price is never returned without knowing its currency. There is no currency mismatch to guard against, because rates and surcharge tiers carry no currency of their own.
- **City whose country cannot be determined**: Its region has no country recorded, so the currency is unknown and the quote is refused as not configured — even when complete city-level settings exist, because the currency comes from the country.
...
- **FR-025**: The booking window, the minimum notice and the surcharge tiers MUST each be resolved for the city that owns the identified zone, in this order: (1) values defined for that city; otherwise (2) values defined for the country the city belongs to. Each of the three settings is resolved independently. When a setting is defined at neither level, the system MUST refuse the request with a distinct reason stating that the service is not configured for that area, and MUST NOT return a price or fall back to an assumed value. The country's currency is part of that configuration: when the country has no currency defined — or the city's country cannot be determined — the system MUST refuse the request in the same way, naming the currency as missing.

**Booking configuration**

- **FR-026**: The system MUST provide a read-only endpoint that, for a given location, returns what a client may pick: the time zone, the current local time, the bookable local dates (today plus the next `bookingWindowDays − 1` days in the city's time zone), the minimum notice, the slot step, the earliest start a quote would accept right now (`null` when nothing is bookable at the moment), the allowed goalkeeper counts, the allowed durations and the currency.
- **FR-027**: That endpoint MUST resolve the location, the time zone and the configuration with exactly the same rules as the quote (FR-010, FR-022, FR-023, FR-025), so it refuses in exactly the same cases and a start time within what it reports is accepted by the quote. The allowed goalkeeper counts, durations and slot step MUST come from the single set of limits the quote's validation also uses, so the two can never disagree.
...
- **Currency**: The currency is a property of the country (`currency` on the country record), because a country has exactly one; rates and surcharge tiers are plain whole-unit amounts in that currency. Current pricing is in Colombian pesos (COP), whole units. The quote returns the country's currency.
