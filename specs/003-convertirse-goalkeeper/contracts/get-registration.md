# Contract: Get Goalkeeper Registration Status

## `GET /api/goalkeepers/me`

Returns the caller's current goalkeeper registration state: every section's saved values (or `null`), each section's computed completeness, and the overall status. Requires a valid internal access token belonging to a client with a complete client profile (`requireAuth`, `requireClientOnly`, `requireCompleteProfile`).
...
**Status**: `401 Unauthorized` (no token) or `403 Forbidden` (admin account, or client profile not yet complete) — no custom body, matching `requireAuth`/`requireClientOnly`/`requireCompleteProfile`'s existing behavior on every other `/api/goalkeepers` route.
...
- Once `status` is `"active"`, this endpoint keeps returning the same shape (read from the now-locked `GoalkeeperRegistration`, which holds identical values to the `GoalkeeperProfile` created at activation) — it does not switch to a different response shape.
