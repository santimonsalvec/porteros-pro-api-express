...
**Status**: `200 OK` — body: the full `GoalkeeperRegistrationResponse` (see get-registration.md), with `documentPhotoASubmitted`/`documentPhotoBSubmitted` reflecting whichever side(s) were just uploaded.
...
```json
{ "error": "already_active", "message": "Your goalkeeper profile is already active; document photos can no longer be changed here." }
```
