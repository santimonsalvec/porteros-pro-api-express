...
**Status**: `200 OK` — body: the full `GoalkeeperRegistrationResponse` (see get-registration.md), reflecting the merge.
...
```json
{
  ...
  "fieldErrors": { "birthDate": "You must be at least 18 years old to register as a goalkeeper." }
}
```
...
```json
{ "error": "duplicate_document", "message": "This identification document is already registered to a goalkeeper account." }
```
