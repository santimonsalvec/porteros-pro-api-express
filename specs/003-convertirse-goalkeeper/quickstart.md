# Quickstart: Become a Goalkeeper — Progressive Registration & Activation
...
curl -s http://localhost:3000/api/goalkeepers/document-types | jq
...
curl -s -X PATCH http://localhost:3000/api/goalkeepers/me/physical-data \
...
curl -s -X PATCH http://localhost:3000/api/goalkeepers/me/location \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
...
curl -s -X PATCH http://localhost:3000/api/goalkeepers/me/identification \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
...
curl -s -X POST http://localhost:3000/api/goalkeepers/me/activate -H "Authorization: Bearer $TOKEN" | jq
# → 409 { "error": "goalkeeper_profile_incomplete", "missingSections": ["identification"] }

# Upload both document photos
curl -s -X POST http://localhost:3000/api/goalkeepers/me/document-photo \
...
curl -i -X PATCH http://localhost:3000/api/goalkeepers/me/availability \
...
curl -s -X POST http://localhost:3000/api/goalkeepers/me/cancel -H "Authorization: Bearer $OTHER_TOKEN" | jq
# → 200, { "status": "not_started", ... } — all previously saved data and any uploaded photos are gone

curl -s http://localhost:3000/api/goalkeepers/me -H "Authorization: Bearer $OTHER_TOKEN" | jq
...
npm run test:http     # supertest against /api/goalkeepers/*, fake-backed — no real Cloudinary/Mongo calls
```
