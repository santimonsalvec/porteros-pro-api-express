...
- Q: Minimum age to register as a goalkeeper? → A: 18 years — standard legal adulthood, no special justification for a higher bar.
- Q: Must a document number be unique across goalkeeper registrations? → A: Enforced — the combination of document type and document number must not have been registered before; a submission matching an already-registered (document type, document number) pair is rejected as a duplicate.
- Q: Can a section be edited through this feature once the goalkeeper profile is already active? → A: No — this feature's data-entry capability (saving section data) only operates while the registration is not yet active; once active, any attempt to modify a section through it is rejected. The Goalkeeper Registration is a temporary/draft entity that exists only up to activation: activation reads its completed data and uses it to establish the goalkeeper's active, permanent record. Any later changes to that active record are the responsibility of a separate, future active-profile-management capability, out of scope here.
- Q: What format/size validation applies to identification document photos? → A: Reuse the existing system-wide image validation rules as-is (currently JPEG/PNG/WEBP/HEIC/HEIF, size cap set by system configuration) — no goalkeeper-specific override.
- Q: What happens to the draft Goalkeeper Registration record after activation? → A: Retained, locked — it stays as a historical/audit trail once activation consumes it into the active Goalkeeper Profile, but nothing in it can be edited or re-activated again.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Save registration progress one section at a time (Priority: P1)

A client who already has an account and a client profile decides they also want to work as a goalkeeper. They don't have every piece of information on hand right away — a document photo might require finding their ID, a location might require being at home to confirm the address. They fill in and save whichever section they're ready for today (say, physical data), close the app, and come back another day — sometimes weeks later — to fill in the next section (say, location), without losing anything they already saved.
...
1. **Given** a client with no goalkeeper registration yet started, **When** they submit valid data for one section, **Then** that section is saved and marked complete, and the other three sections remain in whatever state they were in before (untouched).
...
7. **Given** a client's goalkeeper profile is already active, **When** they attempt to save data for any section through this registration capability, **Then** the system rejects the attempt — an active goalkeeper's data is managed through a separate capability, not this one.

---

### User Story 2 - Activate goalkeeper profile once all sections are complete (Priority: P2)

Once a client has saved all four sections — identification (including both document photos), physical data, location, and availability — they can activate their goalkeeper profile. Activation is the moment their registration effort becomes real: their profile becomes visible to clients who are searching for goalkeepers to hire.
...
1. **Given** a client has completed all four sections, **When** they request activation, **Then** their goalkeeper profile becomes active and is now discoverable by clients searching for goalkeepers.
...
3. **Given** a client activates their goalkeeper profile, **When** they continue using the app as a client (e.g., searching for or requesting goalkeepers themselves), **Then** their client capabilities and existing client profile remain fully intact and unaffected — activation adds a new capability, it does not replace or reset the client account.
4. **Given** a client's goalkeeper profile has been activated, **When** another client searches for available goalkeepers, **Then** only the approximate location (city, state/department, country, and neighborhood if provided) is shown — never the precise coordinates or the full verified address captured during registration.

---

### User Story 3 - Cancel an in-progress registration (Priority: P3)

A client who started the goalkeeper registration process changes their mind before finishing, and wants to abandon it entirely rather than leave it half-finished. Cancelling wipes out everything they had saved so far, including any uploaded document photos, giving them a clean slate.
...
**Independent Test**: Can be fully tested by having a client save data and photos in one or more sections, cancelling the registration, and confirming none of that data or those photos are retrievable afterward and the registration resets to a fresh, not-started state; and separately, by confirming an already-active goalkeeper profile cannot be cancelled through this capability.
...
2. **Given** a client's goalkeeper profile is already active, **When** they attempt to cancel through this capability, **Then** the system refuses the request — deactivating or removing an already-active goalkeeper profile is a separate capability, out of scope here.
...
- **FR-001**: System MUST allow an authenticated client who already has a client profile to begin goalkeeper registration, consisting of four independently trackable sections: identification, physical data, location, and availability.
...
- **FR-003**: System MUST persist previously saved section data and uploaded document photos indefinitely (with no automatic expiration) until the client either activates their goalkeeper profile or explicitly cancels the registration.
...
- **FR-011**: System MUST reject a birth date that is in the future or that indicates the client is younger than 18 years old, the minimum required age to register as a goalkeeper.
...
- **FR-016**: System MUST prevent a client from activating their goalkeeper profile unless all four sections are complete, and MUST clearly indicate every section still incomplete when activation is attempted prematurely.
- **FR-017**: Upon successful activation, System MUST make the client's goalkeeper profile discoverable to other clients searching for goalkeepers.
- **FR-018**: System MUST NOT expose a goalkeeper's precise coordinates or their full/formatted verified address to any client-facing view or search result — only the approximate location (city, state/department, country, and neighborhood/zone if provided) may be shown; the precise coordinates and formatted address are for internal verification and coverage-radius calculation only.
- **FR-019**: Activating a goalkeeper profile MUST NOT alter, remove, or replace the client's existing client-facing profile or capabilities — a single account MUST be able to hold both the client and goalkeeper capabilities simultaneously.
...
- **FR-021**: System MUST refuse a cancellation request once a client's goalkeeper profile is already active; deactivating or removing an active goalkeeper profile is a separate capability, out of scope for this feature.
...
- **FR-023**: System MUST reject an identification submission whose combination of document type and document number matches one already registered by any other goalkeeper registration (in progress or active) — a given identity document may only ever be registered once across the system.
- **FR-024**: System MUST reject any attempt to save section data through this registration capability once the client's goalkeeper profile is already active; an active goalkeeper's data is out of this feature's reach, reserved for a separate, future active-profile-management capability.
- **FR-025**: Upon successful activation, System MUST read the registration's completed section data and use it to establish the goalkeeper's active, permanent record — the Goalkeeper Registration itself is a temporary/draft entity whose data-entry capability exists only up to the point of activation.
- **FR-026**: Upon successful activation, System MUST retain the now-locked Goalkeeper Registration record as a historical/audit trail; it MUST NOT be deleted, further edited, or re-activated after that point.

### Key Entities

- **Goalkeeper Registration**: One per client, a draft record tracking their progressive path toward becoming a goalkeeper, whose data-entry capability exists only up to activation. Holds the values captured so far for each of the four sections (each may be partially or fully populated, or entirely empty), the computed completeness of each section, and a status (not started, in progress, or active). While not yet activated, its data — including any uploaded document photos — is deleted in full if the client cancels. Once activated, its data is used to establish the Goalkeeper Profile and the record itself becomes permanently locked (retained as a historical/audit trail, never edited, deleted, or re-activated again). The combination of document type and document number MUST be unique across all Goalkeeper Registrations (in progress or active) — the same identity document cannot be registered more than once.
- **Goalkeeper Profile**: The active, permanent record establishing a client as a discoverable goalkeeper, created from a Goalkeeper Registration's completed data at the moment of activation. Once it exists, the originating Goalkeeper Registration's data-entry capability no longer applies — any future changes to a Goalkeeper Profile belong to a separate, out-of-scope active-profile-management capability. A client and their Goalkeeper Profile belong to the same underlying account as their existing client profile but are separate, additive concerns.
- **Identification Document Photo**: One per side ("A"/"B") of a client's identification document, associated with a client's Goalkeeper Registration. Privately stored — never publicly accessible. Replacing one discards the prior photo for that side.
...
- **SC-003**: Once a client successfully activates, their goalkeeper profile becomes discoverable to clients searching for goalkeepers without any additional action required from the newly activated goalkeeper.
...
- **SC-007**: 0% of client-facing searches or profile views for an active goalkeeper ever reveal that goalkeeper's precise coordinates or full verified address.

## Assumptions

- The client authentication and client-profile system already in place is reused as-is; this feature introduces no new way to sign in or establish identity, and requires an existing, authenticated client profile as a prerequisite for starting goalkeeper registration.
- Minimum age to register as a goalkeeper is 18 years (confirmed via clarification), a new, explicit rule for this feature — no existing minimum-age rule was found elsewhere in this system to reuse.
...
- Editing an already-active goalkeeper profile (as opposed to a still-in-progress registration) is out of scope for this feature; this specification covers only the path from "not started" through "in progress" to "active," and the one-time cancellation of an in-progress registration. Ongoing management of an active goalkeeper profile is assumed to be handled by a separate, future capability.
- How the client-search experience that discovers active goalkeepers actually ranks, filters, or presents results (beyond respecting the location-privacy rule in FR-018) is assumed to be a separate, existing or future capability that this feature only feeds by making a goalkeeper profile discoverable once active — the search/matching experience itself is not designed here.
- Document photos reuse this system's existing image format and size validation as-is (confirmed via clarification) — no goalkeeper-specific override on accepted formats or maximum file size.
